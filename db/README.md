...
`postgres-store.mjs` is the optional workspace-state adapter. Set `DATABASE_URL` to enable it; otherwise the Node API continues using its file-backed adapter. The adapter expects `schema.sql` to have been applied, stores the versioned workspace JSON in `workspace_state`, and performs a row lock plus ETag check inside a transaction. Corrective actions are now synchronized into the normalized `corrective_actions` table in that same transaction, with non-column fields preserved in `metadata`; evidence, inspections, and CAPAs remain on the JSON bridge until their foreign-key mappings are migrated.
