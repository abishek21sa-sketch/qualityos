(function startQualityOsLocalAnalysis() {
  ...
  const schema = [
    ...
    { key: 'instrument', label: 'Instrument / gage', hint: 'Optional; do not mix measurement systems.' },
    { key: 'operator', label: 'Operator / inspector', hint: 'Optional traceability field.' },
    { key: 'calibrationDue', label: 'Calibration due date', hint: 'Optional date; expired rows are flagged.' },
  ...
  const scopeKeys = ['characteristic', 'part', 'lot', 'unit', 'instrument'];
  ...
  function scopeMarkup() {
    ...
    for (const key of ['part', 'lot', 'unit', 'instrument']) {
      if (!Number.isInteger(state.mapping[key]) || state.mapping[key] < 0) continue;
      const values = columnValues(key);
      if ((key === 'unit' || key === 'instrument') && values.length > 1) pieces.push(scopeSelect(key, key === 'unit' ? 'Unit · never pool' : 'Instrument · never pool', values));
      else pieces.push(scopeSelect(key, key === 'part' ? 'Part / process' : key === 'lot' ? 'Lot / batch' : key === 'unit' ? 'Unit' : 'Instrument / gage', values, true));
    }
    return pieces.join('');
  }
  ...
  function importedMarkup() {
    ...
    return `<div class="ql-file-strip"><div><div class="ql-section-label">Source artifact</div><strong>${esc(file.name)}</strong><span>${formatBytes(file.size)} · ${state.parsed.rows.length} nonblank data rows · local browser memory</span></div><label class="ql-button quiet ql-replace-file">Replace CSV<input type="file" data-file-input accept=".csv,.tsv,text/csv,text/tab-separated-values" hidden /></label></div>
      ...
      <section class="ql-step"><div class="ql-step-no">03</div><div class="ql-step-content"><h2>Choose one comparable population.</h2><p>Do not combine different characteristics, units, parts, or measurement systems in a single control chart.</p><div class="ql-scope-grid">${scopeMarkup()}</div>${!columnValues('characteristic').length ? '<p class="ql-map-hint">No usable characteristic values mapped: enter the measurement name below.</p>' : ''}${!columnValues('unit').length ? '<p class="ql-map-hint">No usable unit values mapped. Add a unit if known; never combine different units.</p>' : ''}${!columnValues('part').length ? '<p class="ql-map-hint">No part / process column mapped. If the file contains multiple processes, the analysis cannot separate them.</p>' : ''}${!columnValues('instrument').length ? '<p class="ql-map-hint">No instrument / gage column mapped. The report will identify measurement-system traceability as unknown.</p>' : ''}${state.filters.part === '__all__' || state.filters.lot === '__all__' ? '<p class="ql-caution">Multiple parts or lots are pooled by your explicit selection. Confirm they represent the same process and measurement system.</p>' : ''}</div></section>
  ...
  function exportReport() {
    ...
    const report = {
      ...
      quality: { inputRows: state.analysis.sourceRowCount, includedRows: state.analysis.count, rejectedRows: state.analysis.rejectedRows, filteredRows: state.analysis.filteredRows, invalidTimestampRows: state.prepared.invalidTimestampRows, missingSubgroupRows: state.prepared.missingSubgroupRows, missingOperatorRows: state.prepared.missingOperatorRows, invalidCalibrationDueRows: state.prepared.invalidCalibrationDueRows, expiredCalibrationRows: state.prepared.expiredCalibrationRows },
